'''
Step 1: Prepare pb & tflite models for testing (Need to use Haoxin's office desktop)
    a. Generate H5 model variations use 
    b. Convert H5 to pb & tflite
    c. pb models are used for energy prediction, tflite models are used for power measuremnet which is the groundtruth
    d. Put the generated pb models in /Users/xiaolong/Library/CloudStorage/Dropbox-GSUDropbox/Xiaolong_Tu/sec23_result/Dataset/Training/Test_models

Step 2: Train kernel-level prediction models - Random Forest
    a. Generate kenerl-level tflite models (Need to use Haoxin's office desktop)
        - Refers to Notion 'How to generate kernel tflite models with configs'
    b. Measure and prepare kernel-level power (Power Monitor) and latency data (TFLite Benchmark)
        - Save csv (current) and txt (latency) files to .../sec23_result/Dataset/Power & .../sec23_result/Dataset/Latency
    c. Pre-process the power and latency data
        - cd .../sec23_result/Dataset/Code
        - Run Power_data_analysis.py (e.g., python Power_data_analysis.py --kernel Conv/conv-bn-relu)
            * It will process the measured current (csv) data
            * It will calculate an average power consumption for each kernel with a specific feature combination
            * It will save the processed power data with corresponding features (including flops and params) to a JSON file
            * The JSON will be saved at .../sec23_result/Dataset/Training/Power/ with a file name formatted as {kernel_name}_power_{current_date}.json
        - Run Latency_data_analysis.py (e.g., python Latency_data_analysis.py --kernel Conv/conv-bn-relu)
            * It will process the benchmarked latency (txt) data
            * It will extract the average latency data in ms
            * It will save the processed latency data with corresponding features (including flops and params) to a JSON file
            * The JSON will be saved at .../sec23_result/Dataset/Training/Latency/ with a file name formatted as {kernel_name}_latency_{current_date}.json
    d. Generate energy dataset based on the preprocessed power and latency data
        - Remove the {current_date} in the saved JSON files for power and latency. The re-named JSON file should be like {kernel_name}_power.json and {kernel_name}_latency.json
        - Run Energy_data_generator.py (e.g., python Energy_data_generator.py --kernel conv-bn-relu)
            * It will calculate the average energy consumption for each kernel by energy(mJ) = power(mW)*latency(ms)/1000 
            * It will generate a JSON file with energy data at .../sec23_result/Dataset/Training/Energy/ with a file name formatted as {kernel_name}_energy_{current_date}.json

    e. Start kernel-level predictor training using generated energy dataset for each kernel
        - Set up training environment
            * if training for test, mark = "Training_test" in sec23_train_final.py (will split the dataset for training 80% and testing 20%)
                ** Copy and paste four JSON files (each kernel) to .../sec23_result/Dataset/Training_test
                ** (1) kernel configs (e.g., conv-bn-relu.json)
                   (2) generated energy dataset (e.g., conv-bn-relu_energy.json)
                   (3) processed latency data (e.g., conv-bn-relu_latency.json)
                   (4) processed power data (e.g., conv-bn-relu_power.json)
            * if training for prediction, mark = "Kernel_training" in sec23_train_final.py
                ** Copy and paste four JSON files to .../sec23_result/Dataset/Kernel_training
                ** (1)~(4) same to above
        - Run python sec23_train_final.py --kernel conv-bn-relu --pred energy --hardware tflite_cpu (change 'energy' to 'latency' or 'power' if aim to train models for latency/power predictions)
            * It will generate pkl predictors for corresponding kernels at .../sec23_result/Dataset/Training_test or /Kernel_training/predictors
            * It will also generate dataset used for training at .../sec23_result/Dataset/Training_test or /Kernel_training/collection. e.g., 
                model_path,         CIN,    COUT,   feature_0(flop),    feature_1(param),   latency
                fc_train_24XN1X,    25088,  4096,   102.762496,         205.524992,         31.53
                ...

Step 3: Use the trained kernel-level predictors to predict energy/power/latency of a tflite CNN model, such as MobilenetV1
    a. Set up evaluation environment for energy prediction
        * Put the CNN pb models at .../sec23_result/Dataset/Training/Test_models
        * Copy and paste the trained kernel-level predictor (pkl files) to .../sec23_result/Dataset/Training/Energy/predictor/{predictor_name}
        * Rename the folder {predictor_name} where the kernel-level predictors are saved, e.g., cortexA76cpu_tflite21
        * Add the new {predictor_name}, version, category, & kernel_predictors in predictors.yaml located at .../sec23_result/Dataset/Training/Energy/predictor/
    b. Repeat the above process for latency and power prediction
    c. Run python sec23_AIEnergy.py --predictor {predictor_name} --version (1.0/2.0/3.0...) --purpose (Energy/Latency/Power) --modelpath Test_models/
        e.g., python sec23_AIEnergy.py --predictor cortexA76cpu_tflite21 --version 1.0 --purpose Energy --modelpath Test_models/

'''

'''
Run Energy/Power/Latency prediction for a pb model
